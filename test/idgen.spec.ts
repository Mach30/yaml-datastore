...
const expected10Skip0Results = ['E16F4F', '506E59', 'A28836', '4B0D2F', '7A1D77', 'B008AA', '058F13', 'CE5CEF', '409FD4', 'FE0F6F']
...
describe("Test Short ID Generation", () => {
  it("should return empty array if numIDs is not an integer", () => {
    const result = generateIDs(3.14, 0);
    expect(result.toString()).to.equal("");
  });
  it("should return empty array if numSkip is not an integer", () => {
    const result = generateIDs(6, 3.14);
    expect(result.toString()).to.equal("");
  });
  it("should return empty array if numIDS and numSkip is not an integer", () => {
    const result = generateIDs(6.28, 3.14);
    expect(result.toString()).to.equal("");
  });
  it("should return empty array if numIDs is less than 1", () => {
    const result = generateIDs(0, 0);
    expect(result.toString()).to.equal("");
  });
  it("should return empty array if numSkip is less than 0", () => {
  ...
  it("should return expected array for skip 5", () => {
    const numSkip = 4
    const result = generateIDs(6, numSkip)
    expect(result.toString()).to.equal(expected10Skip0Results.slice(numSkip).toString());
  });
  it("should return expected array when called twice", () => {
  ...
  it("should return expected array for skip 5 when called twice", () => {
    const numSkip = 4
    const result1 = generateIDs(6, numSkip)
    const result2 = generateIDs(6, numSkip)
    expect(result1.toString()).to.equal(expected10Skip0Results.slice(numSkip).toString());
    expect(result2.toString()).to.equal(expected10Skip0Results.slice(numSkip).toString());
  });
});
